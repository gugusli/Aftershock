...
func _start_update_loop() -> void:
	while is_inside_tree() and GameManager.game_state == GameManager.GameState.PLAYING:
		_update_metrics()
...
func trigger_special_event() -> void:
	...
	match event:
		SpecialEvent.STAMPEDE:
			event_id = EVENT_ID_STAMPEDE
			push_warning("DirectorAI: Evento especial ESTAMPIDA activado")
		SpecialEvent.ARTILLERY_RAIN:
			event_id = EVENT_ID_ARTILLERY_RAIN
			push_warning("DirectorAI: Evento especial LLUVIA DE ARTILLERÍA activado")
		SpecialEvent.KING_OF_HILL:
			event_id = EVENT_ID_KING_OF_HILL
			push_warning("DirectorAI: Evento especial REY DE LA COLINA activado")
		SpecialEvent.TOTAL_DARKNESS:
			event_id = EVENT_ID_TOTAL_DARKNESS
			push_warning("DirectorAI: Evento especial OSCURIDAD TOTAL activado")
		SpecialEvent.ELITE_WAVE:
			event_id = EVENT_ID_ELITE_WAVE
			push_warning("DirectorAI: Evento especial OLEADA DE ÉLITE activado")
		SpecialEvent.CHAOS_DUPLICATION:
			event_id = EVENT_ID_CHAOS_DUPLICATION
			push_warning("DirectorAI: Evento especial DUPLICACIÓN CAÓTICA activado")
	if event_id != "":
		special_event_triggered.emit(event_id)

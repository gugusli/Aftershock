...
func _damage_loop() -> void:
	while dealing_damage and player_ref and is_inside_tree():
		if is_instance_valid(player_ref):
			var damageable = player_ref.get_node_or_null("Damageable")
			if damageable:
				damageable.take_damage(contact_damage)
...
func update_walk_animation(direction: Vector2) -> void:
	...
	# Rotamos el sprite para que mire hacia la dirección del movimiento
	if direction.length() > 0.1:
		sprite_visual.rotation = direction.angle() + PI/2
		
		# Efecto de "caminar" con un pequeño bounce
		var bounce = sin(Time.get_ticks_msec() / 100.0) * 0.1
		sprite_visual.scale.y = 1.0 + bounce
	else:
		sprite_visual.scale.y = 1.0
